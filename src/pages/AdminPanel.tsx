...
import {
  LayoutDashboard, Package, Layers, Users, LogOut, Plus, Pencil, Trash2, Menu, X, ChevronRight
} from "lucide-react";
...
const AdminPanel = () => {
  ...
  const savePackage = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        sort_order: pkgSortOrder,
        product_id: editingPackage?.product_id || selectedProductId!,
      };
      if (editingPackage) {
  ...
  const resetProductForm = () => {
    ...
    setPName(""); setPCategory("Game"); setPSubCategory("Top up"); setPImageUrl(""); setPSortOrder(0);
  };

  const resetPackageForm = () => {
    setEditingPackage(null);
    setPkgName(""); setPkgPrice(""); setPkgSortOrder(0);
  };

  const startEditProduct = (p: any) => {
    setEditingProduct(p);
    setPName(p.name); setPCategory(p.category); setPSubCategory(p.sub_category);
    setPImageUrl(p.image_url || ""); setPSortOrder(p.sort_order);
  };

  const startEditPackage = (p: any) => {
    setEditingPackage(p);
    setPkgName(p.name); setPkgPrice(String(p.price)); setPkgSortOrder(p.sort_order);
  };
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-nav text-nav-foreground transform transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-4 border-b border-nav-foreground/10">
          <h1 className="text-xl font-bold">
            <span className="text-destructive">RG</span> BAZZER
          </h1>
          ...
          {sidebarItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${
                activeTab === item.id ? "bg-primary text-primary-foreground" : "text-nav-foreground hover:bg-nav-foreground/10"
              }`}
            >
              ...
              <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input value={pName} onChange={(e) => setPName(e.target.value)} placeholder="Free Fire TopUp (BD)" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-muted-foreground mb-1 block">Category</label>
                    <Input value={pCategory} onChange={(e) => setPCategory(e.target.value)} placeholder="Game" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-muted-foreground mb-1 block">Sub Category</label>
                    <Input value={pSubCategory} onChange={(e) => setPSubCategory(e.target.value)} placeholder="Top up" />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-muted-foreground mb-1 block">Image URL</label>
                    <Input value={pImageUrl} onChange={(e) => setPImageUrl(e.target.value)} placeholder="https://..." />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-muted-foreground mb-1 block">Sort Order</label>
                    <Input type="number" value={pSortOrder} onChange={(e) => setPSortOrder(Number(e.target.value))} />
                  </div>
                </div>
                <div className="flex gap-2 mt-4">
                  <Button onClick={() => saveProduct.mutate()} disabled={!pName} className="bg-primary text-primary-foreground">
                ...
                <div className="divide-y divide-border">
                  {products?.map((p) => (
                    <div key={p.id} className="p-4 flex items-center gap-3 hover:bg-secondary/30 transition-colors">
                      ...
                      <button onClick={() => startEditProduct(p)} className="p-1.5 hover:bg-secondary rounded-lg">
                        <Pencil className="w-4 h-4 text-muted-foreground" />
                      </button>
                      <button onClick={() => { setSelectedProductId(p.id); setActiveTab("packages"); }} className="p-1.5 hover:bg-secondary rounded-lg">
                        <ChevronRight className="w-4 h-4 text-muted-foreground" />
                      </button>
                      <button onClick={() => deleteProduct.mutate(p.id)} className="p-1.5 hover:bg-destructive/10 rounded-lg">
          ...
          {activeTab === "packages" && (
            <div className="space-y-4 animate-fade-in">
              ...
              {selectedProductId && (
                <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
                  <h3 className="text-base font-semibold text-foreground mb-4 flex items-center gap-2">
                    <Plus className="w-4 h-4" /> {editingPackage ? "Edit Package" : "Add New Package"}
                  </h3>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      <label className="text-xs font-medium text-muted-foreground mb-1 block">Name</label>
                      <Input value={pkgName} onChange={(e) => setPkgName(e.target.value)} placeholder="25 Diamond 💎" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-muted-foreground mb-1 block">Price (TK)</label>
                      <Input type="number" value={pkgPrice} onChange={(e) => setPkgPrice(e.target.value)} placeholder="22" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-muted-foreground mb-1 block">Sort Order</label>
                      <Input type="number" value={pkgSortOrder} onChange={(e) => setPkgSortOrder(Number(e.target.value))} />
                    </div>
                  </div>
                  <div className="flex gap-2 mt-4">
                    <Button onClick={() => savePackage.mutate()} disabled={!pkgName || !pkgPrice} className="bg-primary text-primary-foreground">
                      {editingPackage ? "Update" : "Add Package"}
                    </Button>
                    {editingPackage && (
                      <Button variant="outline" onClick={resetPackageForm}>Cancel</Button>
                    )}
                  </div>
                </div>
              )}
                ...
                <div className="divide-y divide-border">
                  {packages?.map((pkg: any) => (
                    <div key={pkg.id} className="p-4 flex items-center gap-3 hover:bg-secondary/30 transition-colors">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{pkg.products?.name} • ৳{pkg.price}</p>
                      </div>
                      ...
                      <button onClick={() => startEditPackage(pkg)} className="p-1.5 hover:bg-secondary rounded-lg">
                        <Pencil className="w-4 h-4 text-muted-foreground" />
                      </button>
                      <button onClick={() => deletePackage.mutate(pkg.id)} className="p-1.5 hover:bg-destructive/10 rounded-lg">
                        <Trash2 className="w-4 h-4 text-destructive" />
                      </button>
                    </div>
                  ))}
                  {!packages?.length && (
                    <div className="p-8 text-center text-muted-foreground text-sm">
                      {selectedProductId ? "No packages for this product" : "Select a product to view packages"}
                    </div>
                  )}
                </div>
              </div>
