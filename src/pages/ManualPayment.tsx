...
const ManualPayment = () => {
  ...
  const handleVerify = async () => {
    ...
    if (!trxId.trim()) { toast({ title: "Transaction ID দিন", variant: "destructive" }); return; }
    ...
    <div className="min-h-screen bg-background pb-14">
      ...
      <div className="max-w-lg mx-auto px-3 py-4 space-y-3">
        ...
        <div className="bg-card rounded-xl border border-border p-4 space-y-3">
          ...
          {bkashNumber && (
            <div className="flex items-center justify-between bg-[#E2136E]/10 rounded-lg px-4 py-3">
              ...
              <button onClick={() => copyNumber(bkashNumber, "bKash")} className="p-2 rounded-lg bg-[#E2136E]/20 active:opacity-75">
          ...
          {nagadNumber && (
            <div className="flex items-center justify-between bg-[#F6921E]/10 rounded-lg px-4 py-3">
              ...
              <button onClick={() => copyNumber(nagadNumber, "Nagad")} className="p-2 rounded-lg bg-[#F6921E]/20 active:opacity-75">
        ...
        <div className="bg-card rounded-xl border border-border p-4 space-y-3">
          ...
          <p className="text-[11px] text-muted-foreground">Send Money করার পর যে Transaction ID পাবেন সেটা নিচে লিখুন</p>
          <Input placeholder="যেমন: TrxID ABC123XYZ" value={trxId} onChange={(e) => setTrxId(e.target.value)} className="text-[14px] h-12 font-mono" />
          {errorMsg && (
          ...
          <button onClick={handleVerify} disabled={loading || !trxId.trim()}
