...
import { Globe, Palette, MessageSquare, CreditCard, Save, Loader2, Shield } from "lucide-react";
...
const sections: { id: SettingsSection; label: string; icon: any; description: string }[] = [
  ...
  { id: "payment", label: "Payment", icon: CreditCard, description: "পেমেন্ট নাম্বার সেটিংস" },
...
const colorPresets = [
  { label: "White", value: "0 0% 100%" }, { label: "Light", value: "0 0% 95%" },
  { label: "Green", value: "145 63% 32%" }, { label: "Blue", value: "220 70% 50%" },
  { label: "Purple", value: "270 60% 50%" }, { label: "Red", value: "0 70% 50%" },
  { label: "Orange", value: "25 90% 50%" }, { label: "Teal", value: "180 60% 35%" },
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
        {sections.map((s) => (
          <button key={s.id} onClick={() => setActiveSection(s.id)}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-semibold whitespace-nowrap shrink-0 border ${
              activeSection === s.id ? "bg-primary text-primary-foreground border-primary" : "bg-card text-muted-foreground border-border active:bg-secondary"
            }`}>
        ...
        {fields.map((field) => {
          if (field.type === "toggle") {
            return (
              <div key={field.key} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                ...
                <Switch checked={settingsForm[field.key] === "true"} onCheckedChange={(v) => setSettingsForm(prev => ({ ...prev, [field.key]: String(v) }))} />
              </div>
            );
          }
          if (field.type === "color") {
            return (
              <div key={field.key} className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg border border-border shrink-0" style={{ background: settingsForm[field.key] ? `hsl(${settingsForm[field.key]})` : '#ccc' }} />
                  <Input value={settingsForm[field.key] || ""} onChange={(e) => setSettingsForm(prev => ({ ...prev, [field.key]: e.target.value }))} placeholder="220 70% 50%" className="h-9 text-[13px] font-mono flex-1" />
                </div>
                <div className="flex gap-1 flex-wrap">
                  {colorPresets.map((c) => (
                    <button key={c.value} onClick={() => setSettingsForm(prev => ({ ...prev, [field.key]: c.value }))}
                      className={`h-6 px-2 rounded-md text-[9px] font-semibold border active:opacity-75 flex items-center gap-1 ${settingsForm[field.key] === c.value ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"}`}>
                      <span className="w-2 h-2 rounded-full" style={{ background: `hsl(${c.value})` }} /> {c.label}
                    </button>
                  ))}
                </div>
              </div>
          ...
          return (
            <div key={field.key}>
              ...
              {field.type === "textarea" ? (
                <Textarea value={settingsForm[field.key] || ""} onChange={(e) => setSettingsForm(prev => ({ ...prev, [field.key]: e.target.value }))} className="text-[13px] min-h-[60px]" />
              ) : (
                <Input value={settingsForm[field.key] || ""} onChange={(e) => setSettingsForm(prev => ({ ...prev, [field.key]: e.target.value }))} className="h-9 text-[13px]" />
              )}
            </div>
          );
        })}
